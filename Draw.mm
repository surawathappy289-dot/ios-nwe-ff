...
// Extern declarations for color variables
...
- (void)loadUIState {
    ...
    if ([defaults objectForKey:@"ModMenuLanguage"]) {
        ...
        self.currentLanguage = 2; // Default to Thai
    }
...
- (void)viewDidLoad {
    ...
    if (screenWidth <= 375) {
        ...
        panelWidth = kPanelWidthMedium;
        panelHeight = kPanelHeightMedium;
    } else {
...
- (void)setupSidebar {
    ...
    separator.tag = kSidebarSeparatorTag;
...
- (void)setupHeaderBar {
    const CGFloat headerHeight = kHeaderHeight;
    const CGFloat headerButtonWidth = kHeaderButtonWidth;
    ...
    NSArray *titlesTH = @[@"อาimbOT", @"ESP", @"MSL", @"อาวุธ", @"โปรไฟล์"];
    titleLabel.text = titlesTH[MenuTabAimbot];
    titleLabel.font = [UIFont systemFontOfSize:17 weight:UIFontWeightBold];
    titleLabel.textColor = [self accentColor];
    titleLabel.textAlignment = NSTextAlignmentLeft;
    titleLabel.tag = kHeaderTitleTag;
    [self.headerButton addSubview:titleLabel];
    
    const CGFloat btnSize = 44;
    ...
    themeToggle.tag = kThemeToggleTag;
...
- (void)updateAllContentColors {
    ...
    for (UIView *subview in self.contentContainer.subviews) {
        ...
        if (subview.tag >= 1000 && [subview isKindOfClass:[UIView class]]) {
            ...
            UIView *triggerContainer = [self.contentContainer viewWithTag:kSegmentTriggerContainerTag];
if (triggerContainer) {
...
if (targetContainer) {
    ...
    UIButton *leftBtn = [subview viewWithTag:kToggleLeftButtonTag];
    UIButton *rightBtn = [subview viewWithTag:kToggleRightButtonTag];
    ...
    if (rightBtn) {
        ...
        // ===== BUTTON PILLS =====
         ...
         [subview viewWithTag:kSegmentIndicatorBaseTag] != nil) {
    ...
    UIView *indicator = [subview viewWithTag:kSegmentIndicatorBaseTag];
    if (indicator) {
...
- (void)setupContentArea {
    const CGFloat headerHeight = kContentTopInset + kHeaderHeight + kContentBottomInset;
    const CGFloat contentWidth = panelWidth - kSidebarWidth;
    const CGFloat contentHeight = panelHeight - headerHeight;
    ...
    contentClipView.tag = kContentClipViewTag;
...
- (void)updateContentClipMask {
    UIView *contentClipView = [floatingPanel viewWithTag:kContentClipViewTag];
...
- (NSString *)localizedString:(NSString *)english portuguese:(NSString *)portuguese thai:(NSString *)thai {
    if (self.currentLanguage == 2) {
        return thai ? thai : english;
    } else if (self.currentLanguage == 1) {
...
- (CGFloat)createWeaponsContent:(CGFloat)startY 
                       ...
                       padding:(CGFloat)p {
    CGFloat y = startY;
    BOOL isThai = (self.currentLanguage == 2);
    BOOL isPT = (self.currentLanguage == 1);
    
    y = [self addFeaturePillWithDescription:isThai ? @"เปลี่ยน武器เร็ว" : (isPT ? @"Troca Rápida" : @"Fast Switch") 
                                      key:@"FastSwitch" 
                                    value:FastSwitch 
                                 selector:@selector(toggleFastSwitch:) 
                                 description:isThai ? @"เปลี่ยน武器ทันทีโดยไม่มี delay" : (isPT ? @"Troca armas instantaneamente sem delay" : @"Instantly switches weapons without delay")
...
- (CGFloat)createESPContent:(CGFloat)startY 
    ...
    y = [self addFeaturePill:isThai ? @"เปิดใช้งาน ESP" : (isPT ? @"Ativar Esp" : @"Enable") 
                         key:@"Enable" 
                       value:Vars.Enable 
                    selector:@selector(toggleEnable:) 
                         atY:y width:w padding:p];
    y = [self addFeaturePill:isThai ? @"กล่อง ESP" : (isPT ? @"Caixa ESP" : @"ESP Box") 
                         key:@"Box" 
                       value:Vars.Box 
                    selector:@selector(toggleBox:) 
                         atY:y width:w padding:p];
    y = [self addFeaturePill:isThai ? @"เส้น ESP" : (isPT ? @"Linhas ESP" : @"ESP Lines") 
                         key:@"Lines" 
                       value:Vars.lines 
                    selector:@selector(toggleLines:) 
                         atY:y width:w padding:p];
    y = [self addFeaturePillWithDescription:isThai ? @"โครงกระดูก" : (isPT ? @"Esqueleto" : @"Skeleton") 
                                      key:@"Skeleton" 
                                    value:Vars.Skeleton 
                                 selector:@selector(toggleSkeleton:) 
                                 description:isThai ? @"แสดงโครงกระดูกของผู้เล่น" : (isPT ? @"Mostra estrutura óssea do jogador" : @"Shows player skeleton/bone structure")
                                      atY:y width:w padding:p];
    y = [self addFeaturePillWithDescription:isThai ? @"พลังชีวิต" : (isPT ? @"Vida" : @"Health") 
                                      key:@"Health" 
                                    value:Vars.Health 
                                 selector:@selector(toggleHealth:) 
                                 description:isThai ? @"แสดงแถบพลังชีวิตเหนือ ESP" : (isPT ? @"Exibe barra de vida acima do ESP" : @"Displays player health bar above ESP")
                                      atY:y width:w padding:p];
    y = [self addFeaturePillWithDescription:isThai ? @"ระยะทาง" : (isPT ? @"Distância" : @"Distance") 
                                      key:@"Distance" 
                                    value:Vars.Distance 
                                 selector:@selector(toggleDistance:) 
                                 description:isThai ? @"แสดงระยะทางถึงผู้เล่นเป็นเมตร" : (isPT ? @"Mostra distância até jogadores em metros" : @"Shows distance to players in meters")
                                      atY:y width:w padding:p];
    y = [self addFeaturePillWithDescription:isThai ? @"ชื่อ ESP" : (isPT ? @"Nome ESP" : @"ESP Name") 
                                      key:@"Name" 
                                    value:Vars.Name 
                                 selector:@selector(toggleName:) 
                                 description:isThai ? @"แสดงชื่อผู้เล่นเหนือกล่อง ESP" : (isPT ? @"Mostra nomes dos jogadores acima das caixas ESP" : @"Shows player names above ESP boxes")
                                      atY:y width:w padding:p];
    y = [self addFeaturePillWithDescription:isThai ? @"เส้นขอบ ESP" : (isPT ? @"Contorno ESP" : @"ESP Outline") 
                                      key:@"Outline" 
                                    value:Vars.Outline 
                                 selector:@selector(toggleOutline:) 
                                 description:isThai ? @"เพิ่มเส้นขอบรอบกล่อง ESP" : (isPT ? @"Adiciona borda ao redor das caixas ESP" : @"Adds outline border around ESP boxes")
                                      atY:y width:w padding:p];
    y = [self addFeaturePillWithDescription:isThai ? @"จำนวนศัตรู" : (isPT ? @"Contagem Inimigos" : @"EnemyCount") 
                                      key:@"EnemyCount" 
                                    value:Vars.enemycount 
                                 selector:@selector(toggleenemycount:) 
                                 description:isThai ? @"แสดงจำนวนศัตรูที่อยู่ใกล้เคียง" : (isPT ? @"Mostra número total de inimigos próximos" : @"Shows total number of nearby enemies")
                                      atY:y width:w padding:p];
    
    y = [self addFeaturePillWithDescription:isThai ? @"เอฟเฟกต์เรืองแสง" : (isPT ? @"Efeito Brilho" : @"Glow Effect") 
                                      key:@"Glow" 
                                    value:Vars.Glow 
                                 selector:@selector(toggleGlow:) 
                                 description:isThai ? @"เพิ่มเส้นขอบเรืองแสงรอบศัตรู" : (isPT ? @"Adiciona contorno brilhante ao redor dos inimigos" : @"Adds glowing outline around enemies")
                                      atY:y width:w padding:p];
    
    // ESP Color Selection
    y = [self addSectionLabel:isThai ? @"สี ESP" : (isPT ? @"Cores ESP" : @"ESP Colors") 
                          atY:y width:w padding:p];
    
    y = [self addColorPickerSegment:isThai ? @"สีเส้น" : (isPT ? @"Cor Linha" : @"Line Color") 
                         colorType:0 
                              atY:y width:w padding:p];
    
    y = [self addColorPickerSegment:isThai ? @"สีกล่อง" : (isPT ? @"Cor Caixa" : @"Box Color") 
                         colorType:1 
                              atY:y width:w padding:p];
    
    y = [self addColorPickerSegment:isThai ? @"สีโครงกระดูก" : (isPT ? @"Cor Esqueleto" : @"Skeleton Color") 
                         colorType:2 
                              atY:y width:w padding:p];
    
    y = [self addColorPickerSegment:isThai ? @"สีชื่อ" : (isPT ? @"Cor Nome" : @"Name Color") 
                         colorType:3 
                              atY:y width:w padding:p];
...
- (CGFloat)createAimbotContent:(CGFloat)startY 
    ...
    y = [self addFeaturePill:isThai ? @"อาimbOT" : (isPT ? @"Aimbot" : @"Aimbot") 
                         key:@"Aimbot" 
                       value:Vars.Aimbot 
                    selector:@selector(toggleAimbot:) 
                         atY:y width:w padding:p];
    
    y = [self addFeaturePillWithDescription:isThai ? @"ลากผ่านผนัง" : (isPT ? @"Puxar em Paredes" : @"Aim Visible") 
                                      key:@"AimVisible" 
                                    value:Vars.Aimsilent69 
                                 selector:@selector(toggleAimVisible:) 
                                 description:isThai ? @"แสดงอาimbOT ใน killcam และ replays" : (isPT ? @"Mostra mira em killcam e replays" : @"Shows aimbot in killcam and replays")
                                      atY:y width:w padding:p];
    
    y = [self addFeaturePillWithDescription:isThai ? @"ลากยิง" : (isPT ? @"AimKill" : @"Aim Kill") 
                                      key:@"AimKill" 
                                    value:Vars.playertakedamage2T 
                                 selector:@selector(toggleAimKill:) 
                                 description:isThai ? @"ยิงศัตรูอัตโนมัติเมื่อเล็ง" : (isPT ? @"Mata inimigos automaticamente ao mirar" : @"Automatically kills enemies when aiming at them")
                                      atY:y width:w padding:p];
    
    y = [self addFeaturePill:isThai ? @"ละเลยบอท" : (isPT ? @"Ignorar Bot" : @"Ignore Bot") 
                         key:@"IgnoreBot" 
                       value:IgnoreBot 
                    selector:@selector(toggleIgnoreBot:) 
                         atY:y width:w padding:p];
    
    y = [self addFeaturePill:isThai ? @"ละเลยผู้เล่นที่ล้ม" : (isPT ? @"Ignorar Derrubados" : @"Ignore Knocked") 
                         key:@"IgnoreKnocked2" 
                       value:IgnoreKnocked2 
                    selector:@selector(toggleIgnoreKnocked2:) 
                         atY:y width:w padding:p];
    
    y = [self addSliderPill:isThai ? @"รัศมี FOV" : (isPT ? @"Raio FOV" : @"FOV Radius") 
                      value:Vars.AimFov 
                        min:0 
                        max:500 
    ...
    y = [self addSliderPill:isThai ? @"ความหนา FOV" : (isPT ? @"Espessura FOV" : @"FOV Thickness") 
    ...
    y = [self addColorPickerSegment:isThai ? @"สี FOV" : (isPT ? @"Cor FOV" : @"FOV Color") 
                         colorType:4 
                              atY:y width:w padding:p];
    
    y = [self addSectionLabel:isThai ? @"ประเภท Aimbot" : (isPT ? @"Tipo De Aimbot" : @"Trigger When") 
                          atY:y width:w padding:p];
    y = [self addTriggerWhenSegment:y width:w padding:p];
    
    y = [self addSectionLabel:isThai ? @"เป้าหมาย" : (isPT ? @"Puxada" : @"Target Bone") 
                          atY:y width:w padding:p];
    y = [self addTargetBoneSegment:y width:w padding:p];
    
    y = [self addSectionLabel:isThai ? @"ลำดับความสำคัญ" : (isPT ? @"Prioridade Alvo" : @"Target Priority") 
                          atY:y width:w padding:p];
...
- (CGFloat)createMSLContent:(CGFloat)startY 
    ...
    y = [self addFeaturePillWithDescription:isThai ? @"ความเร็ว" : (isPT ? @"Speed" : @"Speed Bypass") 
                                      key:@"SpeedHack" 
                                    value:SpeedHack 
    ...
    y = [self addFeaturePillWithDescription:isThai ? @"บินผู้เล่น" : (isPT ? @"Voar Player" : @"Up Player") 
                                      key:@"UpPlayer" 
                                    value:Vars.UpPlayer 
                                 selector:@selector(toggleUpPlayer:) 
                                 description:isThai ? @"ยกตำแหน่งผู้เล่นเหนือพื้น" : (isPT ? @"Eleva posição do jogador acima do chão" : @"Elevates player position above ground")
                                      atY:y width:w padding:p];
    
    y = [self addFeaturePillWithDescription:isThai ? @"โทรเลข 8 เมตร" : (isPT ? @"Teleport 8m" : @"Telekill 8m") 
                                      key:@"Teleport8m" 
                                    value:Vars.Telekill 
                                 selector:@selector(toggleTeleport8m:) 
                                 description:isThai ? @"โทรเลขศัตรูภายใน 8 เมตรไปด้านหน้า" : (isPT ? @"Teleporta inimigos dentro de 8m para frente" : @"Teleports enemies within 8m forward")
                                      atY:y width:w padding:p];
    
    y = [self addFeaturePillWithDescription:isThai ? @"ซ่อนแผง" : (isPT ? @"Esconder Painel" : @"Stream Mode") 
                                      key:@"StreamMode" 
                                    value:StreamMode 
                                 selector:@selector(toggleStreamMode:) 
                                 description:isThai ? @"ซ่อนองค์ประกอบอินเทอร์เฟซระหว่างการจับภาพหน้าจอ" : (isPT ? @"Oculta elementos da interface durante captura de tela" : @"Hides UI elements during screen capture")
                                      atY:y width:w padding:p];
    
    y = [self addFeaturePillWithDescription:isThai ? @"รีเซ็ตแขก" : (isPT ? @"Apagar Convidado" : @"Reset Guest") 
                                      key:@"ResetGuest" 
                                    value:Guest2 
                                 selector:@selector(toggleResetGuest:) 
                                 description:isThai ? @"รีเซ็ตข้อมูลและค่ากำหนดบัญชีแขก" : (isPT ? @"Redefine dados e configurações da conta convidada" : @"Resets guest account data and settings")
                                      atY:y width:w padding:p];



    y = [self addFeaturePillWithDescription:isThai ? @"เซ็ตยาเร็ว" : (isPT ? @"Apagar Convid้า" : @"Fast Medkit") 
                                      key:@"fmedkit" 
                                    value:fmedkit 
                                 selector:@selector(toggleFastMedkit:) 
                                 description:isThai ? @"เซ็ตยาเร็ว" : (isPT ? @"Fast Medkit" : @"Fast Medkit")
                                      atY:y width:w padding:p];
...
- (CGFloat)addTriggerWhenSegment:(CGFloat)y width:(CGFloat)w padding:(CGFloat)p {
    ...
    NSArray *titles;
    if (isThai) {
        titles = @[@"ตลอดเวลา", @"ขณะยิง", @"เล็ง"];
    } else if (isPT) {
        titles = @[@"Sempre", @"Atirando", @"Mira"];
    } else {
        titles = @[@"Always", @"Firing", @"Scope"];
    }
...
- (void)triggerSegmentTapped:(UIButton *)sender {
    NSInteger newIndex = sender.tag - kSegmentButtonBaseTag;
    ...
    // Find container
    UIView *container = [self.contentContainer viewWithTag:kSegmentTriggerContainerTag];
    if (!container) return;
    ...
    // Move indicator
    UIView *indicator = [container viewWithTag:kSegmentIndicatorTriggerTag];
...
- (CGFloat)addTargetBoneSegment:(CGFloat)y width:(CGFloat)w padding:(CGFloat)p {
    ...
    if (isThai) {
        ...
        titles = @[@"Cabeça", @"Pescoço", @"Corpo"];
    } else {
        titles = @[@"Head", @"Neck", @"Body"];
    }
    CGFloat buttonWidth = (w - 8) / 3.0;
...
- (void)targetSegmentTapped:(UIButton *)sender {
    ...
    // Find container
    UIView *container = [self.contentContainer viewWithTag:kSegmentTargetContainerTag];
    if (!container) return;
    ...
    // Move indicator
    UIView *indicator = [container viewWithTag:kSegmentIndicatorTargetTag];
    CGFloat buttonWidth = (container.frame.size.width - 8) / 3.0;
...
- (CGFloat)addTargetPrioritySegment:(CGFloat)y width:(CGFloat)w padding:(CGFloat)p {
    ...
    NSArray *titles;
    if (isThai) {
        titles = @[@"ใกล้ที่สุด", @"HP ต่ำสุด", @"ยิงหัว"];
    } else if (isPT) {
        titles = @[@"Mais Próximo", @"Menor HP", @"Tiro na Cabeça"];
    } else {
        titles = @[@"Closest", @"Lowest HP", @"Headshot"];
    }
    CGFloat buttonWidth = (w - 8) / 3.0;
...
- (CGFloat)addLanguageSegment:(CGFloat)y width:(CGFloat)w padding:(CGFloat)p {
    ...
    NSArray *titles = @[@"ภาษาไทย", @"English", @"Português"];
    CGFloat buttonWidth = (w - 8) / 3.0;
...
- (CGFloat)addColorPickerSegment:(NSString *)title 
    ...
    BOOL isPT = (self.currentLanguage == 1);
    if (isThai) {
        colorNames = @[@"ขาว", @"แดง", @"น้ำเงิน", @"เขียว", @"ชมพู"];
    } else if (isPT) {
        colorNames = @[@"Branco", @"Vermelho", @"Azul", @"Verde", @"Rosa"];
    } else {
        colorNames = @[@"White", @"Red", @"Blue", @"Green", @"Pink"];
    }
...
- (void)colorSegmentTapped:(UIButton *)sender {
    ...
    NSInteger colorType = (tag - kSegmentColorContainerBaseTag * 100) / 100;
    NSInteger colorIndex = (tag - kSegmentColorContainerBaseTag * 100) % 100;
    ...
    // Find container
    UIView *container = [self.contentContainer viewWithTag:kSegmentColorContainerBaseTag + colorType];
    if (!container) return;
    ...
    // Move indicator
    UIView *indicator = [container viewWithTag:kSegmentIndicatorColorBaseTag + colorType];
...
- (void)languageSegmentTapped:(UIButton *)sender {
    ...
    // Find container
    UIView *container = [self.contentContainer viewWithTag:kSegmentLanguageContainerTag];
    if (!container) return;
...
- (void)prioritySegmentTapped:(UIButton *)sender {
    ...
    // Find container
    UIView *container = [self.contentContainer viewWithTag:kSegmentPriorityContainerTag];
    if (!container) return;
...
- (CGFloat)addMenuColorPickerSegment:(NSString *)title 
    ...
    NSArray *colorNames;
    if (isThai) {
        colorNames = @[@"แดง", @"น้ำเงิน", @"เขียว", @"ชมพู"];
    } else if (isPT) {
        colorNames = @[@"Vermelho", @"Azul", @"Verde", @"Rosa"];
    } else {
...
- (void)menuColorSegmentTapped:(UIButton *)sender {
    ...
    // Find container
    UIView *container = [self.contentContainer viewWithTag:kSegmentMenuColorContainerTag];
    if (!container) return;
    ...
    // Move indicator
    UIView *indicator = [container viewWithTag:kSegmentIndicatorMenuColorTag];
...
- (CGFloat)createProfileContent:(CGFloat)startY 
    ...
    y = [self addProfileRow:isThai ? @"FPS" : (isPT ? @"FPS" : @"FPS") 
                 valueLabel:&fpsLabel 
    ...
    y = [self addProfileRow:isThai ? @"เวลา" : (isPT ? @"Tempo" : @"Time") 
    ...
    // Device
    y = [self addProfileTextRow:isThai ? @"อุปกรณ์" : (isPT ? @"Dispositivo" : @"Device") 
                          value:[[UIDevice currentDevice] model] 
                            atY:y width:w padding:p];
    
    // iOS Version
    y = [self addProfileTextRow:isThai ? @"iOS" : (isPT ? @"iOS" : @"iOS") 
                          value:[[UIDevice currentDevice] systemVersion] 
                            atY:y width:w padding:p];
    
    // Device Name
    y = [self addProfileTextRow:isThai ? @"ชื่อ" : (isPT ? @"Nome" : @"Name") 
   ...
   // NSString *versionDisplay = isSupported ? appVersion : [NSString stringWithFormat:@"%@ (%@)", appVersion, isPT ? @"Não suportado" : @"No supported"];
    ...
    // y = [self addProfileTextRowColored:isThai ? @"เวอร์ชัน" : (isPT ? @"Versão" : @"Version") 
    //                             value:versionDisplay 
    //                              color:versionColor 
    //                                atY:y width:w padding:p];
    
    // Menu Color Theme Selection
    y = [self addSectionLabel:isThai ? @"ธีมเมนู" : (isPT ? @"Tema do Menu" : @"Menu Theme") 
                          atY:y width:w padding:p];
    
    y = [self addMenuColorPickerSegment:isThai ? @"สีเมนู" : (isPT ? @"Cor do Menu" : @"Menu Color") 
                              atY:y width:w padding:p];
    ...
    if (license) {
        @try {
            ...
            NSString *udidDisplay = isThai ? @"ไม่ทราบ" : (isPT ? @"Desconhecido" : @"Unknown");
            if ([license respondsToSelector:@selector(deviceUDID)]) {
                NSString *udid = [license performSelector:@selector(deviceUDID)];
                udidDisplay = udid ?: (isThai ? @"ไม่ทราบ" : (isPT ? @"Desconhecido" : @"Unknown"));
            }
            y = [self addProfileTextRowCopyable:isThai ? @"UDID" : (isPT ? @"UDID" : @"UDID") 
                                         value:udidDisplay 
            ...
            NSString *authStatus = isThai ? (isAuthorized ? @"อนุญาต" : @"ไม่อนุญาต") : (isPT ? (isAuthorized ? @"Autorizado" : @"Não Autorizado") : (isAuthorized ? @"Authorized" : @"Not Authorized"));
            UIColor *authColor = isAuthorized ? [UIColor systemGreenColor] : [UIColor systemRedColor];
            y = [self addProfileTextRowColored:isThai ? @"สถานะ" : (isPT ? @"Status" : @"Status") 
                                         value:authStatus 
                                         color:authColor 
                                           atY:y width:w padding:p];
            ...
            if (isAuthorized && remainingTime > 0) {
                ...
                y = [self addProfileTextRow:isThai ? @"เวลาที่เหลือ" : (isPT ? @"Tempo Restante" : @"Remaining Time") 
                                     value:timeDisplay 
                                       atY:y width:w padding:p];
            }
        } @catch (NSException *exception) {
    ...
    // Language selector
    y = [self addSectionLabel:isThai ? @"ภาษา" : (isPT ? @"Idioma" : @"Language") 
                          atY:y width:w padding:p];
...
- (void)customSegmentTapped:(UIButton *)sender {
    ...
    UIView *indicator = [container viewWithTag:kSegmentIndicatorBaseTag];
    if (!indicator) return;
...
- (void)toggleFastSwitch:(UISwitch *)s {
    ...
    static void *switchAddr = NULL;
    
    if (FastSwitch) {
        switchAddr = (void *)getRealOffset(0x1051467BC);
        void *addr[] = { switchAddr };
        void *func[] = { (void *)switchfast };
        if (!Zexis(addr, func, 1)) {
            FastSwitch = false;
            s.on = NO;
        } else {
            wasswitch = true;
        ...
        if (wasswitch && switchAddr) {
            if (!ZexisUnhook(switchAddr)) {
                NSLog(@"Failed to unhook FastSwitch");
            }
            wasswitch = false;
            switchAddr = NULL;
        }
...
- (void)toggleResetGuest:(UISwitch *)s { 
    ...
    static void *guestAddr = NULL;
    
    if (Guest2) {
        guestAddr = (void *)getRealOffset(0x10126E688);
        void *addr[] = { guestAddr };
        ...
        if (wasGuestHooked && guestAddr) {
            if (!ZexisUnhook(guestAddr)) {
                NSLog(@"Failed to unhook Guest");
            }
