...
bool Zexis(void *address[], void *function[], int count) {
    if (count > 6 || count <= 0 || !address || !function) return false;
    ...
    if (!initialized) {
        ...
        kr = task_set_exception_ports(mach_task_self(), EXC_MASK_BREAKPOINT, server, EXCEPTION_STATE | MACH_EXCEPTION_CODES, ARM_THREAD_STATE64);
        if (kr != KERN_SUCCESS) {
            mach_port_destroy(mach_task_self(), server);
            return false;
        }
        ...
        pthread_t thread;
        if (pthread_create(&thread, NULL, exception_handler, NULL) != 0) {
            mach_port_destroy(mach_task_self(), server);
            return false;
        }
    ...
    for (int i = 0; i < count; i++) {
        if (!address[i] || !function[i]) return false;
